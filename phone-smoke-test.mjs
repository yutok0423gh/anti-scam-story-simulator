...
await touchEnd();
await wait(650);
assert(await evaluate('document.querySelector(".opening-name-overlay #openingNameForm") !== null'), 'First unlock did not ask the player to choose a name');
assert(await evaluate('document.querySelector(".opening-sheet")?.textContent.includes("怎么称呼你")'), 'Opening name prompt did not explain its purpose');
assert(await evaluate('document.querySelector("#systemNavigation").classList.contains("is-hidden")'), 'System navigation remained visible during required name setup');
await click('#systemHome');
assert(await evaluate('document.querySelector(".opening-name-overlay") !== null'), 'Home button bypassed the required opening name');
await capture('simulator-opening-name');
await evaluate(`(() => { const input = document.querySelector('#openingNameInput'); input.value = '小晴'; document.querySelector('#openingNameForm').requestSubmit(); })()`);
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).profile.name === "小晴"'), 'Opening name was not saved');
...
await command('Page.navigate', { url: `${pageBase}/` });
await wait(320);
assert(await evaluate('document.querySelector(".opening-name-overlay") === null && document.querySelector("#homeScreen").classList.contains("is-active")'), 'Saved opening name was requested again after reload');
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).profile.name === "小晴"'), 'Opening name did not survive reload');
await command('Page.navigate', { url: `${pageBase}/README.md` });
await wait(180);
await evaluate(`(() => { const saved = JSON.parse(localStorage.getItem('polyu_simulator_phone_v1')); saved.profile.name = ''; localStorage.setItem('polyu_simulator_phone_v1', JSON.stringify(saved)); })()`);
await command('Page.navigate', { url: `${pageBase}/` });
await wait(320);
const unnamedResume = await evaluate('JSON.stringify({ name: JSON.parse(localStorage.getItem("polyu_simulator_phone_v1"))?.profile?.name, unlocked: JSON.parse(localStorage.getItem("polyu_simulator_phone_v1"))?.unlocked, overlay: document.querySelector("#overlayLayer")?.innerHTML.slice(0, 120), href: location.href })');
assert(await evaluate('document.querySelector(".opening-name-overlay") !== null'), `An older unnamed save was allowed to bypass the opening name: ${unnamedResume}`);
await evaluate(`(() => { const input = document.querySelector('#openingNameInput'); input.value = '小晴'; document.querySelector('#openingNameForm').requestSubmit(); })()`);
assert(await evaluate('document.querySelector(".opening-name-overlay") === null && document.querySelector(".opening-sheet") === null'), 'Older saved progress was forced through the opening brief again after naming');
await command('Page.navigate', { url: `${pageBase}/?preview=home&simTime=10:00` });
await wait(450);

assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).version === 17'), 'Scenario state was not migrated to version 17');
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).callVoiceLanguage === "yue"'), 'Older state did not default to Cantonese call dialogue');
...
const homeTodoCount = await evaluate('document.querySelectorAll("#homeTodoList .home-todo-item").length');
assert(homeTodoCount === 2, `Desktop to-do widget did not keep optional inbox items out of the task list (${homeTodoCount} rendered)`);
...
assert(await evaluate('document.querySelector("#soundToggle") === null'), 'Sound control still appears in the phone status bar');
...
assert(await evaluate('document.querySelector("#appTitle").textContent === "设置名字" && document.querySelector("#playerNameInput") !== null'), 'First Settings entry did not ask for a name');
await evaluate(`(() => { const input = document.querySelector('#playerNameInput'); input.value = '小晴'; document.querySelector('#playerNameForm').requestSubmit(); })()`);
assert(await evaluate('document.querySelector("#appTitle").textContent === "设置" && document.querySelector(".settings-profile-copy strong").textContent === "小晴"'), 'Saved name did not appear in Settings');
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).profile.name === "小晴"'), 'Player name was not persisted');
await click('[data-action="settings-page"][data-value="profile"]');
await click('[data-action="settings-page"][data-value="name"]');
assert(await evaluate('document.querySelector("#playerNameInput").value === "小晴"'), 'Profile did not allow editing the saved name');
await evaluate(`(() => { const input = document.querySelector('#playerNameInput'); input.value = 'Alex'; document.querySelector('#playerNameForm').requestSubmit(); })()`);
assert(await evaluate('document.querySelector(".settings-profile-copy strong").textContent === "Alex"'), 'Edited name did not update the profile card');
assert(await evaluate('document.querySelector("#appContent").textContent.includes("语言与地区") && document.querySelector("#appContent").textContent.includes("声音与语音")'), 'Settings root did not expose the grouped destinations');
await capture('simulator-settings-root');
await click('[data-action="settings-page"][data-value="sound"]');
assert(await evaluate('document.querySelector("[data-action=settings-toggle-sound]").getAttribute("aria-checked") === "true"'), 'Sound setting is not enabled by default');
await click('[data-action="settings-toggle-sound"]');
assert(await evaluate('document.querySelector("[data-action=settings-toggle-sound]").getAttribute("aria-checked") === "false"'), 'Settings sound switch did not mute');
await click('[data-action="settings-toggle-sound"]');
assert(await evaluate('document.querySelector("[data-action=settings-toggle-sound]").getAttribute("aria-checked") === "true"'), 'Settings sound switch did not unmute');
await click('[data-action="settings-choice"][data-value="voice"]');
assert(await evaluate('document.querySelectorAll("[data-action=set-call-voice]").length === 3'), 'Call language picker did not offer Cantonese, Mandarin and English');
await capture('simulator-settings-call-language-picker');
await click('[data-action="set-call-voice"][data-value="en"]');
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).callVoiceLanguage === "en"'), 'English call language was not persisted');
assert(await evaluate('document.querySelector("[data-action=settings-choice][data-value=voice]").textContent.includes("英语")'), 'Selected English call language was not shown in Settings');
await click('[data-action="settings-choice"][data-value="voice"]');
await click('[data-action="set-call-voice"][data-value="zh-CN"]');
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).callVoiceLanguage === "zh-CN"'), 'Mandarin call language was not persisted');
await click('[data-action="settings-choice"][data-value="voice"]');
await click('[data-action="set-call-voice"][data-value="yue"]');
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).callVoiceLanguage === "yue"'), 'Cantonese call language was not restored');
await capture('simulator-settings-sound');
await click('#systemBack');
assert(await evaluate('document.querySelector("#appTitle").textContent === "设置"'), 'Back from Sounds did not return to Settings root');
await click('[data-action="settings-page"][data-value="locale"]');
await click('[data-action="settings-choice"][data-value="language"]');
assert(await evaluate('document.querySelector(".settings-choice-sheet") !== null'), 'Language picker did not open as a bottom sheet');
await capture('simulator-settings-language-picker');
await click('[data-action="set-language"][data-value="en"]');
assert(await evaluate('document.querySelector("#appTitle").textContent === "Language & Region"'), 'English interface language did not apply');
...
assert(await evaluate('document.querySelector("#appContent").textContent.includes("Format preview")'), 'Settings controls were not localized');
await click('[data-action="settings-choice"][data-value="region"]');
await click('[data-action="set-region"][data-value="US"]');
assert(await evaluate('document.querySelector(".settings-locale-specimen").textContent.includes("Tuesday, August 11")'), 'US date format did not apply');
assert(await evaluate('document.querySelector(".settings-locale-specimen").textContent.includes("AM")'), 'US time format did not apply');
...
await click('#systemBack');
assert(await evaluate('document.querySelector("#appTitle").textContent === "Settings"'), 'System back did not return from Language & Region to Settings root');
await click('[data-action="settings-page"][data-value="time"]');
assert(await evaluate('document.querySelector(".settings-policy-group").textContent.includes("1×") && document.querySelector(".settings-policy-group").textContent.includes("4×")'), 'Time progression page did not explain all speeds');
await capture('simulator-settings-time');
await click('#systemBack');
await click('[data-action="settings-page"][data-value="about"]');
assert(await evaluate('document.querySelector(".settings-about-mark").textContent.includes("2.0.0")'), 'About page did not show the simulator version');
await capture('simulator-settings-about');
await click('#systemBack');
await click('[data-action="confirm-reset-day"]');
assert(await evaluate('document.querySelector(".dialog-sheet").textContent.includes("Restart today")'), 'Restart action did not require confirmation');
await click('[data-action="close-overlay"]');
await click('#systemBack');
...
assert(await evaluate('document.querySelector(".todo-day-hero h2").textContent === "My Day"'), 'To Do-style day heading did not switch to English');
assert(await evaluate('document.querySelector(".todo-task-row").textContent.includes("Collect exchange application documents")'), 'Dynamic task content did not switch to English');
assert(await evaluate('document.querySelectorAll(".task-step").length === 0'), 'Task screen still reveals a prescribed sequence of steps');
...
await click('#systemHome');
await click('#appGrid [data-open-app="mail"]');
assert(await evaluate('document.querySelectorAll(".outlook-bottom-nav button").length === 2 && !document.querySelector("[data-action=mail-search]")'), 'Mail search entry is still present');
await click('[data-action="open-mail"][data-id="mail-parcel"]');
...
assert(await evaluate('document.querySelector("#appTitle").textContent === "语言与地区"'), 'Chinese interface language did not restore');
...
assert(await evaluate('document.querySelector("#appContent").textContent.includes("Hi, Alex")'), 'PolyULife home did not use the player name');
...
await wait(1600);
assert(await evaluate('document.querySelector(".call-turn.caller:last-child p").textContent.includes("請問你想查咩") || document.querySelector(".call-turn.caller:last-child p").textContent.includes("頭先係咪打過嚟查文件")'), 'Hall call greeting did not reflect the call history');
...
assert(await evaluate('document.querySelector(".call-conversation").dataset.callPhase === "checking"'), 'Hall lookup did not enter a neutral waiting state');
await click('[data-action="call-minimize"]');
assert(await evaluate('document.querySelector("#activeCallBar").textContent.includes("对方正在查资料")'), 'Waiting call was not kept active when minimised');
await wait(1850);
await click('#activeCallBar [data-action="call-resume"]');
assert(await evaluate('document.querySelector(".call-turn.caller:last-child p").textContent.includes("尾號7305")'), 'Hall lookup did not finish while the call was minimised');
...
assert(await evaluate('document.querySelector(".call-conversation").dataset.callPhase === "checking"'), 'Full tracking lookup answered instantly');
await wait(1850);
...
await click('#systemHome');
await click('#appGrid [data-open-app="tasks"]');
assert(await evaluate('Boolean(document.querySelector(".todo-task-row[data-task=parcel] [data-action=collect-parcel]"))'), 'Collection action did not unlock after confirmation');
assert(await evaluate('!document.querySelector(".todo-day-page").textContent.includes("取得完整运单号")'), 'Task screen exposed the verification path');
await click('[data-action="collect-parcel"]');
assert(await evaluate('document.querySelector(".todo-task-row[data-task=parcel]").classList.contains("done")'), 'Parcel task did not complete');
await click('#systemHome');
...
await evaluate(`(() => { const input = document.querySelector('#callReplyInput'); input.value = '你是阿明吗？'; document.querySelector('#callReplyForm').requestSubmit(); })()`);
assert(await evaluate(`(() => {
  const variant = JSON.parse(localStorage.getItem('polyu_simulator_phone_v1')).contactVariant;
  const reply = document.querySelector('.call-turn.caller:last-child p').textContent;
  return variant === 'fake' ? reply.includes('阿明') : reply.includes(variant === 'real' ? '阿杰' : '阿倫');
})()`), 'Caller did not respond to a wrong guessed name according to their identity');
...
await evaluate(`(() => { const input = document.querySelector('#callReplyInput'); input.value = '你刚才说什么？'; document.querySelector('#callReplyForm').requestSubmit(); })()`);
assert(await evaluate('document.querySelector(".call-turn.caller:last-child p").textContent.includes("阿明")'), 'Caller did not remember the name the player guessed');
await click('[data-action="call-minimize"]');
await click('#systemHome');
await click('#appDock [data-open-app="contacts"]');
...
await click('[data-action="call-number"][data-id="call-unknown"]');
await wait(1600);
assert(await evaluate('document.querySelector(".call-turn.caller:last-child p").textContent.includes("又係我")'), 'Calling the same number again did not acknowledge the earlier call');
await evaluate(`(() => { const input = document.querySelector('#callReplyInput'); input.value = '你刚才说什么？'; document.querySelector('#callReplyForm').requestSubmit(); })()`);
assert(await evaluate('document.querySelector(".call-turn.caller:last-child p").textContent.includes("阿明")'), 'Returning caller forgot the player’s earlier guess');
await click('[data-action="end-call"]');
await click('#systemHome');
await click('#appDock [data-open-app="contacts"]');
...
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).balance === 3000'), 'Declining an optional event changed the balance');

await click('#systemHome');
await click('#appGrid [data-open-app="tasks"]');
assert(await evaluate('document.querySelectorAll(".todo-task-row").length === 2'), 'Inbox decisions leaked into the required task list');
assert(await evaluate('Array.from(document.querySelectorAll(".todo-task-row")).every(el => el.classList.contains("done"))'), 'Not all daily tasks completed');
await click('[data-action="end-day"]');
...
assert(await evaluate('JSON.parse(localStorage.getItem("polyu_simulator_phone_v1")).profile.name === "Alex"'), 'Restart today did not preserve the player name');
unlockPoint = await elementCenter('#unlockButton');
await touchStart(unlockPoint);
await touchEnd();
await wait(420);
assert(await evaluate('document.querySelector(".opening-sheet") !== null'), 'A real touch tap did not unlock the phone');
assert(await evaluate('document.querySelector(".opening-name-overlay") === null'), 'Restart today asked for the preserved name again');
await click('[data-action="start-day"]');
