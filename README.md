...
- 未接来电回拨流程；可预设粤语、普通话（国语）或英语电话对白，粤语优先使用预录音频。
- Outlook 风格邮件收件箱与详情页；PolyU 情境邮件使用英文原文，并可从发件人右侧的 `…` 菜单选择翻译。
- 电话、短信、邮件、联系人、银行与 PolyULife 等应用界面；浏览器采用手机版 Microsoft Edge 式地址栏、搜索结果和底部工具栏。
- 面向学生的真实感情境：研究邀请、活动缴费、包裹通知、冒充熟人及账号核验等。
- 一部分邮件和活动只是可选信息，不会全部被包装成任务。
- 08:30–17:30 的事件时间轴支持暂停、1×、2×、4×及跳到下一事件；1×遇到任何通知暂停，2×只在重要通知或日程暂停，4×不中断，离开页面期间不补算时间。
- iOS 分层设置页，集中管理简体中文/英文界面、国家或地区格式、声音及粤语/普通话/英语通话语言，并提供时间规则说明和安全的重新开始确认。
- 设置顶部提供“我的”资源账本：新的一天以 HK$3,000 开始，玩家在资金、时间和成长值之间取舍，并可选择最多两个发展方向获得真实机会成长加成。
- PolyULife 新增可预约的真实研究参与场次；预约不立即奖励，14:15开放签到并完成45分钟场次后才结算HK$250津贴和成长值。
- 保留 v1.x 经典剧情模式。
